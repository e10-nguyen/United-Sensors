 ...
 *      1 Stop bits
 *      No Parity
 *      Append CR-LF for clear results
 */
...
#define DPS310_I2C1 &hi2c1
#define DPS310_I2C2 &hi2c2
#define DPS310_I2C3 &hi2c3

#define DPS310_ADDRESS 0x77	//Pulling SDO low to GND or bridging solder jumper on back will change this to 0x76
...
void read_calliberation_data (I2C_HandleTypeDef* hi2c)
{
		  //READ FROM reg 0x10 to 0x21
		  	  uint8_t buffer[18]={0};
		  	  HAL_I2C_Mem_Read(hi2c, 0x77 << 1, 0x10, 1, buffer, 18, 1000);
		  	...
		  	//WRITE TO REG 0x06
		  	  uint8_t prscfg[1];
		  	  prscfg[0] = 0x04;
		  	  HAL_I2C_Mem_Write(hi2c, 0x77 << 1, 0x06, 1, prscfg, 1, 1000);


		  	//WRITE TO REG 0x07
		  	  uint8_t tmpcfg[1];
		  	  tmpcfg[0] = 0x04;
		  	  HAL_I2C_Mem_Write(hi2c, 0x77 << 1, 0x07, 1, tmpcfg, 1, 1000);


		  	//WRITE TO REG 0x09
		  	  uint8_t cfgreg[1];
		  	  cfgreg[0] = 0x0C;
		  	  HAL_I2C_Mem_Write(hi2c, 0x77 << 1, 0x09, 1, cfgreg, 1, 1000);

		  	//WRITE 0x07 TO REG 0x08
		  	  uint8_t meascfg[1];
		  	  meascfg[0] = 0x07;
		  	  HAL_I2C_Mem_Write(hi2c, 0x77 << 1, 0x08, 1, meascfg, 1, 1000);
...
void DPS310_GetPress (I2C_HandleTypeDef* hi2c)
{
				...
				//READ REG 0x03 TO REG 0x05
			  		  uint8_t tmp[3]={0};
			  		  HAL_I2C_Mem_Read(hi2c, 0x77 << 1, 0x03, 1, tmp, 3, 1000);
			  	...
			  	//READ REG 0x00 TO REG 0x02
			  		  uint8_t prs[3]={0};
			  		  HAL_I2C_Mem_Read(hi2c, 0x77 << 1, 0x00, 1, prs, 3, 1000);
			  	...
			  	//CALC PRESSURE
			  		  ...
			  		  sprintf(buffer, "hPa difference from launch: %f\n", dif);
			  		  uprintf(buffer);
...
void DPS310_Start (void)
{
	...
	read_calliberation_data(&hi2c2);
	read_calliberation_data(&hi2c3);
}

void DPS310_Poll (void)
{
	DPS310_GetPress(&hi2c1);
	DPS310_GetPress(&hi2c2);
	DPS310_GetPress(&hi2c3);
}
